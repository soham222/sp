...
function loadHtml5LightBox(jsFolder) {
    (function($) {
        $.fn.html5lightbox = function(options) {
            ...
            if (inst.options.limitfocus) {
                ...
                $(window).on("html5lightbox.lightboxclosed", function() { $(document).off("keydown", inst.disableKeyOutsideOfLightbox) })
            }
            inst.disableEscKey = function(isFullscreen) {
                if (isFullscreen) inst.disableesckeyinfullscreen = true;
                else setTimeout(function() { inst.disableesckeyinfullscreen = false },
                    1E3)
            };
            ...
            inst.readNodeData =
                function(node, parentNode) {
                    ...
                    if (container.length > 0) { if (container.find(".lightboxtitle").length > 0) newItem[ELEM_TITLE] = container.find(".lightboxtitle").text(); if (container.find(".lightboxdescription").length > 0) newItem[ELEM_DESCRIPTION] = container.find(".lightboxdescription").text() }
            ...
            inst.createMarkup = function() {
                ...
                if (inst.options.responsivebarheight) { var winH = inst.getWinH(); if (winH <= inst.options.smallscreenheight) inst.options.barheight = inst.options.barheightonsmallheight }
                ...
                if (inst.options.loadingusefont) {
                    inst.$loading.css({ "top": "50%", "left": "50%", transform: "translate(-50%, -50%)", "font-size": "24px", "line-height": "44px", padding: "10px", color: inst.options.loadingfontcolor, "background-color": inst.options.loadingfontbg });
                ...
                if (inst.options.titlestyle == "right" || inst.options.titlestyle == "left") {
                    ...
                    inst.$elemData.css({ position: "absolute", margin: 0, padding: 0, bottom: inst.options.bordersize + "px", left: inst.options.bordersize + "px", right: inst.options.bordersize + "px", "background-color": "#333", "background-color": "rgba(51, 51, 51, 0.6)" });
                ...
                if (inst.options.titlestyle == "inside" && inst.options.showonmouseoverinside) inst.$lightboxBox.hover(function() { if (inst.currentElem[ELEM_TYPE] == 0 || !inst.options.showinsidetitleforimageonly) inst.$elemData.animate({ opacity: 1 }, 400) }, function() {
            ...
            inst.hideNavArrows = function() {
                var showPrev = false;
                var showNext = false;
                if (inst.options.inGroup) { if (inst.options.arrowloop || !inst.options.arrowloop && inst.options.prevElem < inst.options.curElem) showPrev = true; if (inst.options.arrowloop || !inst.options.arrowloop && inst.options.nextElem > inst.options.curElem) showNext = true }
                if (!showPrev) { inst.$prev.css({ display: "none" }); if (inst.$prevfullscreen) inst.$prevfullscreen.css({ display: "none" }) }
                if (!showNext) {
            ...
            inst.clickHandler = function() {
                ...
                var dataoptions = {};
                $.each($this.data(),
                    function(key, value) { dataoptions[key.toLowerCase()] = value });
            ...
            inst.getWinH = function() { return inst.options.insideiframe && window.self != window.top && inst.options.iframesamedomain ? $(parent.window).height() : $(window).height() };
            ...
            inst.loadElem = function(elem) {
                ...
                if (inst.options.pushwindowhistory && window.history && window.history.pushState) {
                    ...
                    window.onhashchange = function() { if (!window.location.hash || window.location.hash.indexOf("#html5lightbox_") < 0) inst.finish() }
                }
                if (inst.options.googleanalyticsaccount && window._gaq) window._gaq.push(["_trackEvent", "Lightbox", "Open",
                    elem[ELEM_HREF]
                ]);
            ...
            inst.showTitle = function(t, description) {
                ...
                if (text.length > 0) text =
                    '<div class="html5-title">' + text + "</div>";
            ...
            inst.showVideo = function(elem) {
                ...
                inst.resizeLightbox(dataW, dataH, true, function() {
                    ...
                    if (isHTML5) {
                        ...
                        inst.embedFlash($(".html5lightbox-video", inst.$image), inst.options.jsfolder + "html5boxplayer.swf", "transparent", { width: dataW, height: dataH, jsobjectname: "html5Lightbox", hidecontrols: inst.options.videohidecontrols ? "1" : "0", hideplaybutton: "0", videofile: videoFile, hdfile: "", ishd: "0", defaultvolume: inst.options.defaultvideovolume, autoplay: inst.options.autoplay ? "1" : "0", loop: inst.options.loopvideo ? "1" : "0", errorcss: ".html5box-error" + inst.options.errorcss, id: 0 })
                    }
                    inst.$elem.show();
                    inst.showData()
            ...
            inst.showPDF = function(elem) {
                if (inst.options.enablepdfjs) {
                    ...
                    inst.options.isIPad) inst.showWeb(elem, true);
            ...
            inst.showWeb = function(elem, pdfobject) {
                ...
                inst.resizeLightbox(dataW, dataH, true, function() {
                    ...
                    if (pdfobject) {
                        ...
                        inst.$loading.hide()
                    } else $(".html5lightbox-web",
                        inst.$image).html("<iframe style='margin:0;padding:0;border:0;' class='html5lightbox-web-iframe' width='100%' height='100%' src='" + elem[ELEM_HREF] + "' frameborder='0' webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>");
                    $(".html5lightbox-web-iframe", inst.$image).on("load", function() { inst.$loading.hide() });
                    inst.$elem.show();
                    inst.showData();
                    if (inst.options.autoresizecontent && inst.isSameDomain(elem[ELEM_HREF])) {
                        $(".html5lightbox-web-iframe", inst.$image).data("sameorigin", true);
                        $(".html5lightbox-web-iframe",
                            inst.$image).on("load", function() {
            ...
            inst.resizeWindow = function() {
                ...
                var winWidth = $(window).width();
                var winH = inst.getWinH();
                if (!(inst.options.showtitle && inst.currentElem[ELEM_TITLE] && inst.currentElem[ELEM_TITLE].length > 0 || inst.options.showdescription && inst.currentElem[ELEM_DESCRIPTION] &&
                        inst.currentElem[ELEM_DESCRIPTION].length > 0 || inst.options.inGroup && (inst.options.showplaybutton || inst.options.showtitleprefix))) inst.options.barheight = 0;
                ...
                if (inst.currentElem[ELEM_TYPE] == 5 || inst.currentElem[ELEM_TYPE] == 7 || inst.currentElem[ELEM_TYPE] == 10) {
                    ...
                    keepratio = inst.currentElem[ELEM_TYPE] ==
                        0 ? inst.options.imagekeepratio : true
                }
                ...
                if (inst.options.autoresizecontent && (inst.currentElem[ELEM_TYPE] == 5 || inst.currentElem[ELEM_TYPE] == 7 || inst.currentElem[ELEM_TYPE] == 10)) {
                    ...
                    if (inst.currentElem[ELEM_TYPE] == 7 && $(".html5lightbox-web-iframe", inst.$lightbox).length > 0 && $(".html5lightbox-web-iframe", inst.$lightbox).data("sameoriginloaded")) {
                        var iframe = $(".html5lightbox-web-iframe",
                            inst.$lightbox)[0];
            ...
            inst.calcElemSize = function(sizeObj, keepratio) {
                ...
                if (inst.options.titlestyle == "bottom") h0 -= inst.options.barheight;
                    ...
                    "outside") h0 -= inst.$elemData.outerHeight();
            ...
            inst.adjustDataHeight = function() {
                var winWidth = $(window).width();
                var winH = inst.getWinH();
                var isBottomText = inst.options.titlestyle == "bottom" || inst.options.titlestyle ==
                    "outside" || (inst.options.titlestyle == "left" || inst.options.titlestyle == "right") && winWidth <= inst.options.sidetobottomscreenwidth;
                if (inst.options.responsivebarheight)
                    if (winH <= inst.options.smallscreenheight) inst.options.barheight = inst.options.barheightonsmallheight;
                    else inst.options.barheight = inst.options.barheightoriginal;
                if (isBottomText) {
            ...
            inst.enableSwipe = function() {
                ...
                inst.$lightboxBox.html5lightboxTouchSwipe({
                    ...
                    swipeLeft: function() { if (inst.options.inGroup) inst.gotoSlide(-1) },
                    swipeRight: function() { if (inst.options.inGroup) inst.gotoSlide(-2) }
                })
            };
            inst.hideObjects = function() { $("embed, object").css({ "visibility": "hidden" }) };
            inst.showObjects = function() { $("embed, object").css({ "visibility": "visible" }) };
            ...
            inst.each(function() {
                ...
                if (typeof html5lightbox_options != "undefined" && html5lightbox_options) { if ("autoopen" in html5lightbox_options) autoopen = html5lightbox_options.autoopen; if ("autoopendelay" in html5lightbox_options) autoopendelay = html5lightbox_options.autoopendelay; if ("autoopenonce" in html5lightbox_options) autoopenonce = html5lightbox_options.autoopenonce; if ("autoopenonceexpire" in html5lightbox_options) autoopenonceexpire = html5lightbox_options.autoopenonceexpire }
                autoopen =
                ...
                if (autoopen) {
                    ...
                    if (autoopenonce) {
                        ...
                        for (var i in cookies) {
                            ...
                            if (parts.length && parts[0] == "html5lightboxpopup") { is_popped = true; break }
                        }
                    }
                    if (!is_popped) {
                        setTimeout(function() {
            ...
            if ("html5lightboxshare" in urlParams) { var shareUrl = decodeURIComponent(urlParams["html5lightboxshare"]); var shareLink = $('.html5lightbox[href="' + shareUrl + '"]'); if (shareLink.length > 0) shareLink.click() }
    ...
    (function($) {
        $.fn.lightboxHTML5VideoControls = function(skinFolder, parentInst, videoElem, hideControls, hidePlayButton, defaultVolume, fullscreenNativeControls, html5VideoNoDownload, skinImages, showsubtitle, vttline, showsubtitlebydefault) {
            ...
            try {
                $videoObj.on("canplay", function() { if (showsubtitle) setVttline(vttline) });
